...
- `rahbar` → SELECT everything; INSERT/UPDATE **only** on admin tables (owners, product_categories, product_types, calibres, settings_limits, users). No operational write policy exists for rahbar — that absence *is* the enforcement. **v1.21:** these admin-table policies were tightened from `FOR ALL` to separate INSERT/UPDATE policies (no DELETE) — extending "Never DELETE — void" (below) from operational records to master data too (§3.3/§6.4).
- `menejer` → writes orders/requests · **v1.21: plus INSERT/UPDATE on `owners` alone** (§3.3 — clients only, no other admin table) · `qorovul` → writes weighings · `ombor` → writes storage/pallets/dispatch · `laborator` → writes lab results.
...
### 3.3 Product settings — now split by scope 🔒 (v1.21)

🔒 **Menejer's own scope is clients only** — "a new buyer shouldn't require the owner," since Menejer is the one taking orders. **Mijozlar** (`/menejer/mijozlar`): add a new client, rename an existing one. No deactivate control here (see §6.4's own note on why) and no access to product categories/types/calibres/thresholds at all — those routes don't exist on Menejer's side. Every other piece of "product settings" this section originally named moved to **§6.4 Boshqaruv**, Rahbar's own administration screen, since categories/types/calibres/Konditirskiy and every `settings_limits` threshold are Rahbar-only master data (§2.12).

**Implemented as:** `src/components/ClientsSection.tsx` (shared with §6.4's own client management, `allowDeactivate` prop toggles the one control that differs) + `src/pages/menejer/MenejerClientsTab.tsx`.
...
### 6.4 Boshqaruv (Administration) — the only place he WRITES 🔒 (v1.21, applied)

**Sozlamalar** (`/rahbar/sozlamalar`), mounted alongside the existing Foydalanuvchilar screen (same page-shell convention, no new layout):

- **Mahsulot turkumlari, turlari, kalibrlari** (product categories / types / calibres incl. Konditirskiy, §3.3) — Rahbar-only, one list-rename-deactivate-add section each.
- **Mijozlar (OWNERS)** — client master table (§2.4). Shares its component with Menejer's own §3.3 clients screen; the deactivate control is the only difference between the two, gated by a prop, not a second implementation.
- **Limitlar (thresholds, §2.14)** — every `settings_limits` key, current value shown with its unit and what it governs in plain Uzbek (a bare number with no context is unmaintainable). `Sig'imi (kg/oy)` — the one nullable threshold (§3.2.10) — can be set **and cleared back to unset**; clearing it makes the Rahbar dashboard's utilisation line disappear again, the same as it never having been configured.
- 🔒 **Foydalanuvchilar va rollar** (unchanged, out of scope this task): create logins for menejer/qorovul/ombor/laborator, deactivate leavers, manage other Rahbar accounts.
...
🔒 **RLS tightened alongside this build, not just extended.** The pre-existing `rahbar_admin_*` policies on these four tables (plus `settings_limits`) granted `FOR ALL` — including DELETE — even though SPEC.md's own §2.15 text already documented "INSERT/UPDATE only" as the intended design. Replaced with narrower INSERT+UPDATE-only policies per table, closing that gap rather than opening a new restriction. **New:** Menejer gets INSERT+UPDATE on `owners` alone (create/rename clients only) — deliberately row-level, not column-restricted to `name`, since this app's RLS model shares one Postgres role (`authenticated`) across every app-role and a real column-level restriction would need a trigger or a dedicated RPC, disproportionate for a reversible, UI-mediated, low-stakes gap (Menejer's own clients screen simply never renders a deactivate control). See DECISIONS.md "Rahbar settings (§3.3)" for the full reasoning.

**Implemented as:** `src/pages/rahbar/RahbarSettingsTab.tsx` + `ProductCategoriesSection.tsx`/`ProductTypesSection.tsx`/`CalibresSection.tsx`/`ThresholdsSection.tsx`, `src/components/ClientsSection.tsx` (shared), `src/lib/masterDataAdmin.ts` (shared collision-handling + CRUD helpers). `supabase/migrations/0031_rahbar_settings_schema_and_rls.sql`.
