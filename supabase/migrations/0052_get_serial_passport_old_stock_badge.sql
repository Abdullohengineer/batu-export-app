-- Opening stock, Stage 2 (2026-08-02) -- the original inspect-and-plan
-- task named the passport alongside qoldig'i/client report for "reading as
-- distinct from current stock" (point 7); qoldig'i and the client report
-- got this in Stage 1/2, the passport didn't yet. Adds isOldStock to the
-- 'order' object only -- every other field/CTE in this function is
-- byte-identical to the live definition (confirmed via pg_get_functiondef
