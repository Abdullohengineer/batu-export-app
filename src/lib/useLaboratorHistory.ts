...
export function useLaboratorHistory(filters: LaboratorHistoryFilters) {
  ...
  const { data, isPending, isFetching, error } = useQuery({
    queryKey: queryKeys.laboratorHistory(from, to, scope, ownerId, typeId, calibreId, seriya, verdict),
    queryFn: async ({ signal }): Promise<LaboratorHistoryRow[]> => {
      ...
      const { data: cycles, error: cyclesErr } = cycleIds.length
        ? await supabase.from('wash_cycles').select('id, serial').in('id', cycleIds).abortSignal(signal)
        : { data: [] as { id: string; serial: string }[], error: null }
      ...
      const { data: lines, error: linesErr } = allSerials.length
        ? await supabase
            .from('kirim_lines')
      ...
      const { data: orders, error: ordersErr } = orderIds.length
        ? await supabase.from('kirim_orders').select('order_id, owner_id').in('order_id', orderIds).abortSignal(signal)
        : { data: [] as { order_id: string; owner_id: string }[], error: null }
      if (ordersErr) throw new Error(ordersErr.message)
      const orderById = new Map((orders ?? []).map((o) => [o.order_id, o]))

      const sampledBarcodes = [...new Set(chiqimResults.map((r) => r.sampled_pallet).filter((b): b is string => !!b))]
      const { data: pallets, error: palletsErr } = sampledBarcodes.length
        ? await supabase.from('finished_pallets').select('barcode2, calibre_id').in('barcode2', sampledBarcodes).abortSignal(signal)
        : { data: [] as { barcode2: string; calibre_id: string }[], error: null }
      if (palletsErr) throw new Error(palletsErr.message)
