-- The nakladnoy the Menejer attaches on the KIRIM form ("Mijoz
-- nakladnoyasini biriktirish", kirim_orders.doc_photo, kirim-photos bucket)
-- was captured but never returned by get_serial_passport -- confirmed by
-- reading the function's own 'order' jsonb_build_object, which omitted it.
-- Purely additive: one new key, no other change to the function.
