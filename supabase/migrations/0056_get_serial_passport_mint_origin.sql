-- Opening stock, Stage 3 (2026-08-02): the passport's mint-origin section.
-- A minted serial (origin='internal_reprocess') is "born" from consumed
-- pallets rather than arriving on a truck, so its Buyurtma/Darvoza sections
-- are near-empty and would otherwise leave its whole beginning unexplained.
--
-- mintOrigin joins serial_mint_sources -> finished_pallets to recover each
-- consumed pallet's BOOK weight, which is what makes storage loss visible
-- and derivable at the point of use:
--   storage loss = bookTotalKg - (this serial's moyka_sends)
-- never stored, never folded into wash loss.
--
-- Also adds `notes` (entity_type='moyka', keyed by serial) so the passport
-- surfaces the auto-note the mint writes plus anything Ombor/Laborator
-- added later -- the passport rendered no notes at all before this.
--
-- Everything else is byte-identical to the live definition (confirmed via
-- pg_get_functiondef immediately before writing this).
