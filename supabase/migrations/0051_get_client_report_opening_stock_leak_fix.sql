-- Found via proactive origin-awareness sweep after Stage 2 (per explicit
-- instruction: treat origin-awareness as the default assumption for any
-- read of confirmed_at/finalized_at/received_date/order_date/completed_at,
-- not just the three sites already found -- checked every remaining
-- report-facing view/function that reads those columns).
--
-- get_client_report's loss_totals/loss_output/loss_main leaked the same way
-- yield_rows did in Stage 1: the seed's synthetic 'final' wash_cycles rows
-- (old-washed, minted only to satisfy the hard gate) have completed_date
-- backdated to 2025-01-01 with zero real moyka_sends but real
-- finished_pallets weight. Any client-report query spanning 2025-01-01
-- picks up all 5 old-washed serials via loss_totals, producing
-- processedBreakdown.lossKg = -52210 (sent_kg=0 minus real calibreKg)
-- instead of the correct 0 -- confirmed live before this fix:
-- get_client_report(<Global Export>, '2025-01-01','2025-01-31')
-- ->'raw'->'processedBreakdown' returned
-- {"lossKg":-52210,"calibreKg":52210,"konditirskiyKg":0}.
--
-- quality_record has the same exposure via arrival_date (old-raw, which has
-- no wash_cycle_status='final' to be caught by the loss_totals fix, would
-- still surface with empty intake/delivered lab fields under a fabricated
-- arrival date) -- same origin exclusion added there too.
--
-- Every other CTE in this function is byte-identical to the live
-- definition (confirmed via pg_get_functiondef immediately before writing
-- this) -- only loss_totals and quality_record change.
