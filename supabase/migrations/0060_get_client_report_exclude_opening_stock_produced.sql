-- Client-report fix (2026-08-03, opening-stock audit item 22). Fourth
-- rewrite of this function; the raw three-bucket arithmetic is UNCHANGED --
-- only which pallets count as finished.producedKg/byCalibre.producedKg
-- changes.
--
-- THE BUG: client_pallets (the finished-goods membership CTE added by
-- migration 0058) carries no origin filter at all, so opening_stock's
-- fabricated 2025-01-01 pallets satisfy `received_date between p_from and
-- p_to` whenever a report's window straddles that seed date -- reporting
-- 52,210 kg as genuinely "produced" on a single fabricated day. Measured
-- live before this fix: get_client_report(Global Export, '2025-01-01',
-- '2025-01-01')->finished = {openingKg:0, producedKg:52210, closingKg:52210}
-- -- the entire opening-stock seed read as a real production event.
--
-- THE FIX IS NARROW, NOT A BLANKET EXCLUSION: opening_stock pallets are
-- real, currently-held client inventory and must keep counting in
-- openingKg/closingKg/dispatchedKg (balance and real-departure figures) --
-- excluding them there would understate the client's genuine on-hand stock
-- by 52,210 kg. Only finished_produced_total and finished_produced_by_calibre
-- (the "did production really happen in this period" figures) gain the
-- guard, mirroring the exact `cl.origin != 'opening_stock'` pattern
-- loss_totals/quality_record already use for the raw side. client_pallets
-- itself is otherwise untouched -- membership, departure_date as-of-p_to,
-- voided-pallet handling all stay exactly as migration 0058 left them.
--
-- SECOND BOUNDARY CASE, caught in verification before this ever shipped:
-- finished_opening_total's own `received_date < p_from` is a STRICT
-- inequality, so opening_stock (received_date = 2025-01-01 exactly) fails
-- that test whenever p_from <= 2025-01-01 -- with produced now correctly
-- excluding it too, the 52,210kg would vanish from BOTH buckets for any
-- report starting on or before the fabricated seed date, undercounting
-- instead of overcounting. Per explicit instruction, opening_stock must
-- always count as opening regardless of whether the period straddles
-- 2025-01-01 -- so finished_opening_total/finished_opening_by_calibre gain
-- an `or origin = 'opening_stock'` arm alongside the existing strict-date
-- test. Safe unconditionally: client_pallets' own WHERE already requires
-- `fp.received_date <= p_to` for a row to exist here at all, so an
-- opening_stock row is never reachable for a p_to earlier than 2025-01-01
-- in the first place -- there is no query shape where this could count a
-- period that ended before the stock's own fabricated existence began.
--
-- Every other CTE is byte-identical to the 0058 definition (this file was
-- seeded from it, so the diff IS the change: `ko.origin` added to
-- client_pallets' select list, `and origin != 'opening_stock'` added to the
-- two produced-total CTEs, and `or origin = 'opening_stock'` added to the
-- two opening-total CTEs).
...
-- FIX 3: a Tugallash that happens AFTER p_to must not retroactively add
-- loss to a closed period. Mirrors the guard moykada_total and
-- cumulative_loss_total already apply. All 6 existing final cycles carry a
-- finalized_at, so requiring it is a no-op on current data.
loss_totals as (
...
-- FIX 3b: pallets packed after p_to must not inflate a closed period's
-- output. Same received_date <= p_to pattern client_lines.output_as_of_to_kg
-- already uses.
loss_output as (
  select
...
-- FIX 1 + 2 (migration 0058): membership reconstructed from EVENTS as of
-- p_to, never from current status. A pallet counts as this client's
-- finished stock for the period if it was produced on or before p_to and
-- had not yet departed, been consumed, or been voided as of p_to.
--
-- Departed pallets deliberately STAY in this CTE -- finished_dispatched_total
-- needs them, and finished_opening_total already tests departure_date
-- against p_from, which now reads correctly as "hadn't departed before
-- p_from" because departure_date itself is as-of.
--
-- Note status='dispatched' is not tested: it is confirmed dead (migration
-- 0026's own comment, and 0 rows carry it). Dispatch lives entirely in
-- dispatch_manifest + gate_weighings.completed_at.
--
-- ko.origin added here (2026-08-03) so the two produced-total CTEs below
-- can exclude opening_stock without re-deriving membership -- deliberately
-- NOT used to filter this CTE itself, since opening_stock pallets are real
-- inventory and must stay counted in opening/closing/dispatched.
client_pallets as (
  select
...
-- FIX (2026-08-03, opening-stock audit item 22): opening_stock's fabricated
-- 2025-01-01 received_date must never read as a real production event,
-- regardless of whether the report period happens to straddle that seed
-- date. Real balance (opening/closing) and real departures (dispatched)
-- are unaffected -- only "was this genuinely produced in this period" is
-- guarded, same class of fix as loss_totals/quality_record's existing
-- `origin != 'opening_stock'` guard on the raw side.
...
-- FIX 4: a CHIQIM gate completing after p_to must not retroactively make a
-- dispatch appear inside a closed period.
