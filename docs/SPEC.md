...
- **Qayta yuvish darajasi** *(v1.20 — §3.2.10 Rahbar exceptions)* — a client's monthly re-wash rate (finished serials with a 2nd+ cycle ÷ all finished serials that month) above *X* % (default 20).
...
🔒 **Deliberate date-basis exception to §3.2.3, labelled so it doesn't get "fixed" back.** `Davrda qabul qilingan` and `Davrda olib ketilgan` use arrival/departure dates exactly as §3.2.3 already defines (KIRIM → gate stage 1, CHIQIM → gate stage 2). `Davrda qayta ishlangan` does **not** use `moyka_sends.sent_date` — it uses cycle 1's own completion date (`min(finished_pallets.received_date)` for `wash_cycle=1`). Raw stays "raw" for balance purposes the whole time it's physically in Moyka; it only becomes countable as processed once that wash cycle actually concludes.

🔒 **Raw is set once, at cycle 1, and nothing downstream revises it (§2.13).** `effective_qty` is fixed per serial; `Davrda qayta ishlangan` is derived from `wash_cycle=1` only, always — a re-wash re-send is a new Moyka cycle entry against material already counted, and never adds to it a second time, regardless of how many cycles follow. Also capped at `effective_qty` — confirmed against real data that `moyka_sends` can exceed the recorded raw figure — and when that cap engages, the overage is shown explicitly (actual sent kg, the capped figure, the gap, and which serial), never silently clamped away.
...
**Section B — quality record.** Per serial with *any* activity in the period — arrived, processed (its cycle completed), or dispatched, not arrival-only: a serial delivered this period must show its delivered reading even if it arrived in an earlier one, since that's what the client actually received. Intake moisture/SO₂ + delivered moisture/SO₂/verdict/wash-cycle-number-if->1, the client's own target printed beside each. No SO₂ target → **"Talab yo'q · naturel"** (§5.5.1).
...
🔒 **One row per fully-processed serial, not per event and not per cycle.** A serial appears once its CURRENT (highest) wash cycle is `final` — still-active serials are WIP (§3.2.9), not yield yet. "Raw counted once per serial regardless of cycles" only means something at this per-serial granularity, which is why this view rolls a re-washed serial's every cycle into one row rather than showing cycle 1 and cycle 2 as separate lines.
...
🔒 **Loss: raw counted once, accumulated across every cycle, denominator against raw actually consumed — not summed sends.** Same `all_*`/`live_*` split §3.2.7 established: `all_*` (every pallet a cycle ever produced, voided or not) feeds the **loss kg** figure — summed across every one of the serial's cycles, since a re-wash's own input is the previous cycle's voided output flowing back into Moyka (§2.13), not fresh raw; `live_*` (currently `in_stock` only) feeds **output kg** and the **calibre mix**, so a voided cycle's superseded output is never double-counted alongside whatever later cycle replaced it. **Loss %'s denominator is always `raw_consumed_kg`** — cycle 1's own actual `moyka_sends` total, uncapped, counted once — never the sum of every cycle's own send (0029's original bug, applied here correctly from the start).
...
Intake moisture is the serial's own KIRIM lab reading (§5.5.2); delivered moisture is its **current (highest) cycle's own** CHIQIM reading — same "judge by current cycle" rule the stock-on-hand bug fix (§3.2.6) already established, so a re-wash's dry-matter figure reflects what was actually delivered, not a superseded first attempt. **Computed only where both readings exist.** Where either is missing (an untested arrival, or a washed-but-lab-pending batch — both real, in-progress states, not data gaps), the row shows gross only and says so explicitly ("ma'lumot yo'q") — never estimated or inferred. Pre-build coverage check on the seeded fixture: 12 of 16 real serials (75%) had both readings, the other 4 all legitimately in-progress — dry-matter is a headline column here, not a footnote.

🔒 **Deliberate date-basis distinction from §3.2.7: a serial's row is dated by its LAST cycle's completion, not cycle 1's.** The client report needs cycle-1 dating because it's a period balance sheet that must never retroactively change after being handed to a client (§3.2.7's cross-period-rewash handling exists solely for that reason). This view has no such constraint — it's a live-recomputed summary of one serial's whole story, which isn't truly known until its last cycle finishes. So a re-washed serial's entire yield (raw consumed, output, loss, both cycles) is attributed to the period its last cycle completed in, told once rather than fragmented across two periods the way the client report deliberately fragments it.
