-- EMERGENCY HOTFIX (2026-09-08) — restores client_serial_ledger, dropped by
-- migration 0118, byte-for-byte as it stood in production immediately before
-- that drop (0109's original body, as amended by 0110's Остаток сырья fix —
-- no other migration between 0110 and 0118 touched this function).
--
-- Root cause: 0118 and the ClientPrihodTab.tsx rewrite that stops calling
-- this function are both part of PR #140, which was NOT merged/deployed
-- when 0118 was applied directly to the live project. `main`'s currently
-- deployed frontend still calls client_serial_ledger(p_from_date, p_to_date,
-- p_product_type_id) (src/lib/clientSerialLedger.ts) — a live DB change was
-- shipped ahead of the frontend change it depends on, breaking Приход for
-- every real client user with
-- "Could not find the function public.client_serial_ledger(...) in the
-- schema cache" until this restore.
--
-- This is a STOPGAP, not a reversal of the rewrite: once PR #140 merges and
-- deploys, ClientPrihodTab.tsx no longer calls this function at all, and it
-- becomes genuinely dead code again (confirmed by the same grep/dependency
-- check 0118 itself already did) — safe to drop a second time in a small
-- follow-up migration at that point. Left in place for now rather than
-- coordinating a merge+drop in lockstep, which is exactly the coordination
-- failure that caused this incident.
