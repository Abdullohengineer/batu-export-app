-- Client-report as-of-date fix (2026-08-02). Third rewrite of this
-- function; the raw three-bucket arithmetic is UNCHANGED -- only which
-- pallets/serials are counted changes.
--
-- THE BUG: the report answered historical questions by reading CURRENT
-- status, so re-running a closed period returned different numbers
-- depending on what happened afterwards. Hand a client their June
-- statement, re-run it in September, get different figures. Pre-existing
-- via voided pallets, but Stage 3's re-wash consumption made it routine.
-- Measured live before this fix: get_client_report(Global Export,
-- 2025-01-01 .. 2026-08-01)->finished.closingKg read 51,170 kg, having
-- silently dropped 1,040 kg of pallets that were genuinely in stock for
-- that whole period and were only consumed on 2026-08-02, AFTER p_to.
--
-- FOUR as-of defects fixed, not one -- fixing only the first would have
-- left the report half-stable:
--   1. client_pallets               -- membership from current status
--   2. client_pallets.departure_date -- gate completion with no <= p_to
--   3. loss_totals / loss_output     -- current wash-cycle status, and
--                                       pallets packed after p_to
--   4. period_dispatch_ids           -- gate completion with no <= p_to
-- moykada_total and cumulative_loss_total already guarded finalized_at
-- correctly; #3 just makes loss_totals consistent with its own siblings.
--
-- Every other CTE is byte-identical to the 0051 definition (this file was
-- seeded from it, so the diff IS the change).
...
-- FIX 1 + 2: membership reconstructed from EVENTS as of p_to, never from
-- current status. A pallet counts as this client's finished stock for the
-- period if it was produced on or before p_to and had not yet departed,
-- been consumed, or been voided as of p_to.
--
