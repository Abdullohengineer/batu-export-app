-- Raw dispatch on the serial passport: a new top-level `rawDispatches` array,
-- distinct from the existing pallet-based `dispatches` (per the task's own
-- "distinct entry under dispatches" instruction) -- one row per
-- raw_dispatch_lines event for this serial. Everything else in this
-- function is byte-identical to the current (0041) body.
...
-- Raw dispatch (2026-07-31) -- this serial's own raw-collected events,
-- joined through chiqim_lines/chiqim_requests the same way dispatch_pallets
-- joins through dispatch_manifest/finished_pallets above.
raw_dispatches as (
